...
INTROS = {
    ...
    "Acquisition — high-rate ADC → baseband pipeline": (
        ...
        "`design_halfband` takes an `exact_dc_gain` flag. A half-band "
        "satisfies A(0) + A(0.5) = 1 identically and A(0.5) is a stopband "
        "extremum, so unity DC gain and maximum stopband depth are mutually "
        "exclusive: the default (`False`) takes the deeper stopband and a "
        "DC gain of 1 ± δ, bounded by the same ripple the passband already "
        "accepts. Pass `True` when unity DC gain through cascaded stages "
        "matters more, at a cost of roughly 6 dB.\n\n"
