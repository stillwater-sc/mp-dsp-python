...
INTROS = {
    ...
    "Acquisition — high-rate ADC → baseband pipeline": (
        ...
        "**Known limitation — `design_halfband` ([#117](https://github.com/"
        "stillwater-sc/mp-dsp-python/issues/117)).** The upstream Remez "
        "exchange does not converge correctly, so this designer tops out "
        "near 21 dB of stopband attenuation and gets *worse* with more taps "
        "— 127 taps at `transition_width=0.15` measures −24.7 dB, meaning "
        "the stopband sits above the passband. Where real selectivity "
        "matters, use `fir_lowpass` with a Kaiser or Blackman window, which "
        "reaches 88 dB at 51 taps.\n\n"
        "**`NCO` and `DDC` hold `frequency` and `sample_rate` at the "
        "configuration's state precision** and divide only afterwards, so "
        "absolute rates at RF scale overflow narrow types. Passing "
        "normalized rates — `sample_rate=1.0` with the frequency as a "
        "fraction of it — is well defined for every dtype and means the "
        "same thing, since an oscillator only uses the ratio. Rates that "
        "would produce a non-finite phase increment now raise rather than "
        "yielding silent NaN."
    ),
    "IIR filter design — RBJ biquads": (
    ...
    "FIR filter design": (
        "Window-method designs (`fir_lowpass` and friends) plus the "
        "equiripple `remez*` family and the zero-phase `filtfilt`. All take "
        "`coeff_dtype=` to run the design-time arithmetic at a chosen "
        "precision; taps are stored as float64 either way.\n\n"
        "**Known limitation — the `remez*` designers ([#117](https://github."
        "com/stillwater-sc/mp-dsp-python/issues/117)).** The upstream Remez "
        "exchange does not converge correctly: designs carry a fixed ~2.4 dB "
        "passband ripple with a 1.25 DC gain regardless of tap count, rather "
        "than the equiripple response the method promises. Where passband "
        "flatness matters, use `fir_lowpass` with a Kaiser window — measured "
        "0.00 dB ripple and 88 dB stopband at 51 taps."
    ),
    "Signal generators": (
